...
// 
    ...
    @(test)
    key_map__is_valid__test :: proc(t: ^testing.T) {
        context.logger = log.create_console_logger()
        defer log.destroy_console_logger(context.logger)
        ...
        zero_value: Key_Map
        testing.expect(t, key_map__is_valid(&zero_value) == false)
        testing.expect(t, key_map__is_valid(nil) == false)

        m: Key_Map
        testing.expect(t, key_map__init(&m, 8, allocator) == nil)
        testing.expect(t, key_map__is_valid(&m))

        testing.expect(t, key_map__terminate(&m, allocator) == nil)
        testing.expect(t, key_map__is_valid(&m) == false)
    }