...
// Tests
    ...
    serialization_malformed_snapshot__test :: proc(t: ^testing.T) {
        ...
        // starts after the Snapshot_Header (56 bytes), the entity-id blob
        // (saved_cap * 8, no freed entries here), and the eid_to_disabled_bits
        // blob (saved_cap * 16 * TABLES_MULT bytes — Uni_Bits is a 128-bit
        // bit_set per TABLES_MULT word, added in SNAPSHOT_VERSION 5; Uni_Bits
        // itself is package-private, so this re-derives its size the same way
        // bits.odin does: BIT_SET_VALUES_CAP bits per word, 8 bits per byte);
        ...
        //
            ...
            section_off := 56 + ENTITIES_CAP * 8 + ENTITIES_CAP * uni_bits_size
