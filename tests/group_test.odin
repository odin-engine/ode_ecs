...
// ODE
    ...
    group__verify :: proc(t: ^testing.T, group: ^ecs.Group, pos: ^ecs.Table(Group_Pos), vel: ^ecs.Table(Group_Vel)) {
        ...
        ps := ecs.slice(group, pos)
        vs := ecs.slice(group, vel)
    ...
    group__basic__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, ecs.slice(&group, &extra) == nil)
    ...
    group__ownership__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, ecs.slice(&group_b, &pos) == nil)
    ...
    group__pause_resume__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, ecs.slice(&group, &pos) == nil, "dirty group must not hand out slices")
    ...
    group__pause_resume_group_level__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, ecs.slice(&group, &pos) == nil, "still dirty: pack does not rebuild")
    ...
    group__single_owned_table__test :: proc(t: ^testing.T) {
        ...
        testing.expect(t, ecs.slice(&group, &pos) == nil, "dirty group must not hand out slices while paused")

        testing.expect(t, ecs.resume_packing(&group) == nil)
        testing.expect(t, ecs.group_len(&group) == 8) // 10 - eids[3] - eids[0]

        ps2 := ecs.slice(&group, &pos)
