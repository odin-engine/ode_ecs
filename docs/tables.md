...
`positions.rows` is a plain dense slice, but reach it through `ecs.table_dense_slice(&positions)`
rather than the `rows` field directly:

```odin
for &pos, index in ecs.table_dense_slice(&positions) {
...
> `for &pos in ecs.table_dense_slice(&positions)` iterate the same data, but not with the same
...
> `table_dense_slice` returns the slice by value from a call, giving the loop a fresh local with
> no such aliasing concern. Same idea as `view_dense_slice`/`group_dense_slice`/
