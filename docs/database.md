...
An entity is just an ID (`ecs.entity_id`) — a `bit_field` packing an index (`ix`, 32 bits) and a generation (`gen`, 32 bits). All entity data lives in component [tables](tables.md).
