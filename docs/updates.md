...
**September 2026**
- **new** •  [Growing in place](/docs/overbase.md#growing) — `grow` (also `overbase_grow`) raises an Overbase's entity capacity, or a table's, pair table's or relations table's row capacity, after init. Ids, components, pairs and views are kept, and views re-point their cached pointers. Meant for load boundaries such as hot reload, never the frame loop.
- **new** •  [Flags_Table](/docs/flags_table.md) — up to 128 flags per table (an entity can be in any number of Flags_Tables). `flag`/`unflag`/`has_flag` (also `tag`/`untag`/`has_tag` with a bit), with integer bits or enums. Views filter on flags with `Flags` terms and a `Flags_Op` (`And`, `Or`, `Xor`, `Nor`, `Nand`, `Exact`); `&status` alone means "has any flag". Works with Command_Buffer (`cmd_flag`), Observers (`Flags_Changed`), snapshots, Sync and `pause_packing`/`resume_packing`/`pack`. `Bits` is now public.
